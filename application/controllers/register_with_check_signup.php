<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	function check_database($password)
	 ...
	 function verifysignup(){
			   ...
			   // echo $response=$this->form_validation->run();
			   // die("i m in res");
                    if($this->form_validation->run()== FALSE) {
						echo 'I m in error';
						...
						$this->data['query'] = $this->logo->GetInitData();
						$this->load->view('user_registration',$this->data);
					}else if($this->form_validation->run() == 'invalid_affliate_id'){
					 ...
					 }else if($this->form_validation->run() == 'allready_signup_error'){
	...
	{	
		...
		if($is_username_exist==0)
		{
			...
			// die("cccccc");
			if($result!='invalid_affliate_id' && $result!='allready_signup')
			{	
